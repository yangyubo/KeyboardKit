...
The ``Keyboard`` namespace contains a lot of essential, keyboard-related types and views. By typing `Keyboard.`, Xcode will list all essential types within this namespace.
...
When you use a ``KeyboardView``, you must however use a ``KeyboardStyleService`` instead, since the keyboard view is so complex. You can however apply explicit styles within the ``KeyboardView``'s various view builder parameters.
