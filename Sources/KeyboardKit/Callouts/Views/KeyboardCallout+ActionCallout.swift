...
private extension KeyboardCallout.ActionCallout {
    ...
    func calloutItem(for action: KeyboardAction) -> some View {
        ...
        case .controlCombination(let asciiValue): calloutItem(for: action.inputCalloutText ?? "")
        case .metaCombination(let asciiValue): calloutItem(for: action.inputCalloutText ?? "")
        default: EmptyView()
        }
    }
